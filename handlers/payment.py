...
from database import (
    get_shop_status, get_item_by_id, update_order_status,
    create_order, get_order_details, update_order_delivery,
...
def init_payment_handlers(bot):
    ...
    def handle_buy_button(call):
        try:
            ...
            # Stock ကုန်နေလျှင်
            if item_stock <= 0:
                bot.answer_callback_query(call.id, "❌ ဒီပစ္စည်းက Stock ပြတ်သွားပါပြီ၊၊", show_alert=True)
            ...
            # ဝယ်ချင်သည့် အရေအတွက်ထက် Stock နည်းနေလျှင်
    ...
    def handle_payment_method(call):
        ...
        item = get_item_by_id(item_id)
        if not item:  # ← null check ကို အရင်ဆုံး လုပ်ရမည်
            bot.answer_callback_query(call.id, "❌ ပစ္စည်း ရှာမတွေ့ပါ၊၊", show_alert=True)
            return
    ...
    def handle_screenshot(message):
        ...
        bot.send_photo(
            ...
            caption=admin_text, reply_markup=markup, parse_mode='Markdown'
        )
    ...
    # ၆။ အော်ဒါ အသေးစိတ် ကြည့်ရှုခြင်း
