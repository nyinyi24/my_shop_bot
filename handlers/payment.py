...
# Stock ကုန်သွားတဲ့အခါ Admin ပို့မည့် account ကို စောင့်နေသော order များ
# format: {admin_id: {"target_user_id": ..., "item_name": ..., "order_id": ...}}
waiting_for_account = {}

def init_payment_handlers(bot):

    # ၁။ ဝယ်ယူမည့် ခလုတ်ကို နှိပ်ခြင်း
    ...
    def handle_payment_method(call):
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("🎧 Support", url=SUPPORT_URL),
    ...
    def handle_screenshot(message):
        ...
        # Admin က account info ပို့နေတာ စစ်ဆေးပါ
        if user_id == ADMIN_ID and ADMIN_ID in waiting_for_account:
            return  # text handler က ကိုင်မည်
    ...
    def admin_action(call):
        ...
        if action == "approve":
            ...
            if account_data:
                # Auto-deliver — stock ထဲမှ အကောင့်ရှိသောအခါ
                ...
                print(f"✅ Order #{order_id} auto-delivered to {target_user_id}")

            else:
                # Stock ကုန်နေသောအခါ — Admin ဆီ account info တောင်းခံမည်
                waiting_for_account[ADMIN_ID] = {
                ...
                bot.edit_message_caption(
                    caption + "\n\n⏳ *Status: Waiting Manual Delivery*",
                    call.message.chat.id, call.message.message_id, parse_mode='Markdown'
                )

                msg = bot.send_message(
                    ADMIN_ID,
                    f"⚠️ <b>{item_name}</b> အတွက် Stock ကုန်နေပါသည်၊၊\n\n"
                    f"👤 User ID: <code>{target_user_id}</code>\n\n"
                    f"ဝယ်ယူသူဆီ ပို့ပေးမည့် <b>Account Info (Mail:Pass)</b> ကို အောက်တွင် ရိုက်ထည့်ပေးပါ-",
                    parse_mode='HTML'
                )
                bot.register_next_step_handler(msg, process_manual_delivery, target_user_id, item_name, order_id)
            ...
            reject_msg = (
                ...
                "အဆင်မပြေမှု တစ်စုံတစ်ရာ ရှိပါက Support ခလုတ်ကိုနှိပ်၍ Admin ကို တိုက်ရိုက်ဆက်သွယ် မေးမြန်းနိုင်ပါတယ်ဗျာ၊၊"
            )
    ...
    def process_manual_delivery(message, target_user_id, item_name, order_id):
        ...
        # waiting_for_account မှ ဖယ်ရှားမည်
        if ADMIN_ID in waiting_for_account:
            del waiting_for_account[ADMIN_ID]
        ...
        bot.send_message(target_user_id, delivery_msg, parse_mode='HTML')

        # Admin ဆီ အတည်ပြုချက်
        bot.reply_to(message, f"✅ User <code>{target_user_id}</code> ထံ ပစ္စည်းပို့ဆောင်ပြီးပါပြီ၊၊", parse_mode='HTML')
        print(f"✅ Order #{order_id} manually delivered to {target_user_id}")
    ...
    def my_orders(call):
        ...
        if not orders:
            ...
            bot.edit_message_text(
                "🛒 *ဝယ်ယူထားသည့် မှတ်တမ်းမရှိသေးပါ၊၊*",
                call.message.chat.id, call.message.message_id,
    ...
    def view_order_detail(call):
        try:
            ...
            if order:
                ...
                bot.edit_message_text(
                    detail_text, call.message.chat.id, call.message.message_id,
                    reply_markup=markup, parse_mode='Markdown'
                )
        except Exception as e:
